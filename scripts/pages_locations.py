...
def district_page(region, ds, dn):
    ...
    night_phrase = ('야간 예약 비중이 두꺼운' if any(k in pf['customer'] for k in ['관광','상인','젊은'])
                    else '주간·저녁이 균형 잡힌')
    # v 변형: 4종 FAQ 뱅크 — 행정구별 도어웨이 회피 (질문·답변 문구 다변화)
    faq_banks = [
        # v=0
        [(f"{dn} 마사지 주요 고객층은 누구인가요?",
          f"{dn}의 핵심 수요층은 {pf['customer']}입니다. {pf['land']} 일대를 거점으로 예약이 모입니다."),
         (f"{dn}에서 가장 잘 되는 업종은 무엇인가요?",
          f"{svc} 마사지가 가장 두텁습니다. {pf['market']}"),
         (f"{dn} 교통편이 출퇴근에 편한가요?",
          f"{pf['metro']}로 닿을 수 있어 출퇴근과 고객 방문 모두 수월합니다. 환승 거점에 가까운 매장일수록 유동 손님이 많습니다."),
         (f"{dn}는 야간 근무 비중이 큰가요?",
          f"{dn}는 {night_phrase} 권역입니다. 영업 시간은 매장마다 달라 공고에서 직접 확인하는 것이 정확합니다."),
         (f"{dn} 로드샵 분포는 어떤가요?",
          f"{pf['land']} 인근에 로드샵이 모여 있는 형태입니다. {pf['strategy']}"),
         (f"신입이 {dn}에서 시작해도 괜찮을까요?",
          f"가능합니다. 수습 교육을 운영하는 매장이 있어 체력·적성 확인 후 점차 타임을 늘려 정착할 수 있습니다.")],
        # v=1
        [(f"{dn}에 어떤 손님들이 자주 오나요?",
          f"주된 손님은 {pf['customer']}입니다. {pf['land']} 권역의 생활·업무 동선과 맞물려 예약이 형성됩니다."),
         (f"{dn}에서 단가가 가장 좋은 업종은?",
          f"평균적으로 {svc} 마사지가 단가와 회전율 모두 우위입니다. {pf['market']}"),
         (f"{dn} 출퇴근 동선이 어떻게 되나요?",
          f"{pf['metro']} 라인을 따라 매장이 분포해 이동이 짧습니다. 역세권 매장은 신규 손님 유입이 더 활발합니다."),
         (f"{dn} 평일·주말 어느 쪽이 바쁜가요?",
          f"고객 구성상 {night_phrase} 패턴이 우세하지만, 매장 운영 정책에 따라 다릅니다."),
         (f"{dn}에 가게가 많이 모인 곳은?",
          f"{pf['land']} 일대가 중심입니다. {pf['strategy']}"),
         (f"{dn} 입직 시 주의할 점이 있나요?",
          f"합법 신고 업체인지·근로계약서·정산 방식 세 가지를 입사 전 반드시 확인하세요. 마톡은 합법 업체 공고만 게재합니다.")],
        # v=2
        [(f"{dn} 마사지 시장 분위기는 어떤가요?",
          f"{pf['market']} 핵심 손님은 {pf['customer']}입니다."),
         (f"{dn}에서 {svc} 외에도 수요가 있는 업종은?",
          f"{svc}가 가장 두껍지만, 인근 상권 특성상 다른 업종도 보조 수요가 있습니다. 본인 강점과 매장 운영 방향을 함께 보세요."),
         (f"{dn} 일대 교통 거점은?",
          f"{pf['metro']}이 핵심 동선입니다. 거점에서 도보 10분 이내 매장이 유동 수요 측면에서 유리합니다."),
         (f"{dn} 영업 시간대는 어떻게 되나요?",
          f"고객층상 {night_phrase} 패턴이 흔합니다. 정확한 영업 시간은 공고별로 확인하세요."),
         (f"{dn} 로드샵 운영 환경은?",
          f"{pf['strategy']} 매장 선택 시 휴게·위생·정산 투명성을 함께 살펴보면 장기 근속에 도움이 됩니다."),
         (f"{dn} 신규 입직 절차가 궁금합니다.",
          f"공고 선택 → 매칭 신청 → 조건 사전 조율 → 면접·체험 → 근로계약 작성 순으로 진행됩니다.")],
        # v=3
        [(f"{dn}의 주요 손님 특성을 알려주세요.",
          f"이 권역은 {pf['customer']} 중심으로 수요가 형성됩니다. {pf['land']} 일대가 거점입니다."),
         (f"{dn}에서 가장 권할 만한 업종은?",
          f"{svc} 마사지 수요가 가장 두텁습니다. {pf['market']}"),
         (f"{dn} 매장은 어떤 동선에 모여 있나요?",
          f"{pf['metro']} 동선을 따라 분포합니다. 환승·거점 인접 매장일수록 신규 손님 비중이 큽니다."),
         (f"{dn} 시간대별 예약 흐름은?",
          f"{night_phrase} 운영이 흔합니다. 본인 라이프스타일에 맞춰 매장 영업 시간을 고르세요."),
         (f"{dn} 권역 정착 팁이 있나요?",
          f"{pf['strategy']} 단골 응대와 정산 점검을 꾸준히 하면 수입 변동성을 줄일 수 있습니다."),
         (f"{dn}에서 처음 일을 시작하려면?",
          f"마톡 매칭 신청을 통해 합법 신고 매장의 공고에 우선 지원하세요. 수습·교육 과정이 있는 매장 위주로 보는 것을 권장합니다.")],
    ]
    faqs = faq_banks[v]
    ...
    review_banks = [
        # v=0
        [(f"{dn} {svc} 3개월차 입직 후기", f"{pf['land']} 근처 로드샵에서 일하고 있어요. {pf['customer']} 단골이 늘면서 수입이 안정됐습니다."),
         (f"{dn} 경력 이직 후기", f"{pf['metro']} 접근성이 좋아 출퇴근이 짧아졌고, 정산이 투명해 만족합니다."),
         (f"{dn} 신입 정착기", f"수습 교육이 체계적이라 {svc} 기본기를 빠르게 익혔습니다."),
         (f"{dn} 단골 응대 후기", f"{pf['customer']} 고객이 많아 재방문 관리가 중요했어요. 꾸준히 하니 단골이 생겼습니다."),
         (f"{dn} 파트 근무 후기", "근무 시간을 조율할 수 있어 가정과 병행하기 좋았어요."),
         (f"{dn} 장기 근속 후기", f"{pf['strategy']} 덕분에 오래 일하기 좋은 환경이라 생각합니다.")],
        # v=1
        [(f"{dn} 입직 후 첫 분기", f"{pf['land']} 상권 손님들이 꾸준해서 적응이 빨랐습니다. {svc} 위주로 경력을 쌓고 있어요."),
         (f"{dn} 단가 협상 경험", f"{pf['metro']} 인근 매장이라 객수가 안정적이고, 경력자 단가 협상이 수월했습니다."),
         (f"{dn} 응대 노하우", f"{pf['customer']} 손님은 디테일을 잘 보세요. 인사·청결을 꼼꼼히 챙기니 재방문이 늘었습니다."),
         (f"{dn} 신입에게 한마디", "처음에는 무리한 타임 수보다 기본기 안정이 우선이에요."),
         (f"{dn} 근무 환경 평", "휴게 공간·위생 관리가 잘 되어 있어 컨디션 유지에 도움이 됐습니다."),
         (f"{dn} 1년 이상 근속", f"{pf['strategy']} 그래서 같은 매장에서 1년 넘게 일하고 있습니다.")],
        # v=2
        [(f"{dn} 신입 6개월 회고", f"처음엔 어려웠지만 {svc} 기본 동작을 다지니 단골이 생겼습니다. {pf['land']} 손님층이 안정적이에요."),
         (f"{dn} 출퇴근 만족도", f"{pf['metro']} 라인 덕분에 이동이 짧아 체력 관리가 한결 수월했습니다."),
         (f"{dn} 단골 비중 변화", f"꾸준한 응대로 {pf['customer']} 비중이 늘었고, 월 수입 변동이 줄었어요."),
         (f"{dn} 정산 투명성", "일정산 내역을 매일 확인할 수 있어 안심하고 일했습니다."),
         (f"{dn} 야간·주말 근무 후기", "본인 컨디션에 맞춰 시간대를 고르면 부담이 덜합니다."),
         (f"{dn} 매장 선택 팁", f"{pf['strategy']} 매장 분위기와 정산 방식을 사전에 충분히 묻고 결정하세요.")],
        # v=3
        [(f"{dn} 첫 매장 정착기", f"{pf['land']} 인근 매장에서 시작했어요. 손님이 꾸준해 빠르게 적응했습니다."),
         (f"{dn} 업종 전환 경험", f"{svc} 중심으로 옮긴 뒤 단가와 만족도가 함께 올라갔습니다."),
         (f"{dn} 매장 이동 후기", f"{pf['metro']} 라인 안에서 옮겼더니 출퇴근 시간이 크게 줄었습니다."),
         (f"{dn} 단골 관리 비결", f"{pf['customer']} 손님의 컨디션을 기록해 두면 재방문 응대가 자연스러워져요."),
         (f"{dn} 파트→정규 전환", "수습 기간 동안 능력을 보여주면 정규 전환과 단가 인상이 함께 따라옵니다."),
         (f"{dn} 장기 근속 환경", f"{pf['strategy']} 그래서 동료들도 오래 일하는 분이 많습니다.")],
    ]
    reviews = review_banks[v]
